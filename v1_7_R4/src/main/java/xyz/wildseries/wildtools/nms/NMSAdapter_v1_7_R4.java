...
public final class NMSAdapter_v1_7_R4 implements NMSAdapter {
        ...
        if ((block.d() && !block.isTileEntity()) && (silkTouch || EnchantmentManager.hasSilkTouchEnchantment(player))) {
            ...
            for(int i = 0; i < dropCount; i++) {
                if (world.random.nextFloat() < 1.0F) {
                    ...
                    if (item != null) {
                        //Adds item to drops
                        drops.add(CraftItemStack.asBukkitCopy(new ItemStack(item, 1, block.getDropData(Block.getId(block)))));
                    }
                }
            }
        }
    ...
    public void registerGlowEnchant() {
        class GlowEnchantment extends net.minecraft.server.v1_7_R4.Enchantment {
    ...
    public void setAirFast(org.bukkit.block.Block block) {
        ...
        world.setTypeAndData(block.getX(), block.getY(), block.getZ(), Block.getById(0), 2,  0);
    }
